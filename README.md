...
The API includes abuse guards for high-risk endpoints. `RATE_LIMIT_AUTH_MAX`, `RATE_LIMIT_UPLOAD_MAX`, and `RATE_LIMIT_ADMIN_MUTATION_MAX` control per-window limits for auth, media upload signing, and Admin mutations; `RATE_LIMIT_WINDOW_MS` controls the window, `RATE_LIMIT_STORE=memory` selects the current in-process store, and `RATE_LIMIT_ENABLED=false` disables the guard for trusted internal deployments. When a client is limited the server returns HTTP 429 with a `Retry-After` header and emits a structured rate-limit event that can be wired to logs or metrics.
