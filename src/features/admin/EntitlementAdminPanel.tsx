...
export function EntitlementAdminPanel({
  ...
  const run = async (action: () => Promise<void>, success: string) => {
    ...
    try {
      ...
      setFeedback({ kind: 'error', text: error instanceof Error ? error.message : text('Entitlement operation failed.', '权益操作失败。') })
    } finally {
      setBusy(false)
    }
  }
  ...
  const exportSnapshot = () => run(async () => {
    ...
    downloadJsonArtifact({
      value: snapshot,
      fileName: `personal-entitlements-${new Date().toISOString().slice(0, 10)}.json`,
  ...
  const sweepExpiry = async () => {
    ...
    setFeedback(null)
    try {
      const result = await entitlementService.sweepExpired()
      await refreshAll()
      setFeedback({ kind: 'success', text: text(`${result.expired} grants expired.`, `${result.expired} 个授权已过期。`) })
    } catch (error) {
      console.info('[entitlement-admin]', error)
      setFeedback({ kind: 'error', text: error instanceof Error ? error.message : text('Expiry sweep failed.', '到期扫描失败。') })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <section className="entitlement-admin-panel" data-testid="admin-entitlements-panel">
      <SectionHeader
        ...
        action={<div className="button-row compact-buttons">
          ...
          {canTransition && <button className="ghost-button" type="button" onClick={() => void sweepExpiry()} disabled={busy}><Activity size={16}/>{text('Sweep expiry', '扫描到期')}</button>}
        </div>}
      />
      ...
      {view === 'plans' && <>
        ...
        <div className="entitlement-master-detail"><div className="admin-table">{plans.map((plan) => <button className={`admin-row compact ${selectedPlan?.id === plan.id ? 'selected' : ''}`} type="button" key={plan.id} onClick={() => void selectPlan(plan)}><span className={`status ${plan.status}`}>{statusLabel(plan.status)}</span><strong>{plan.title}</strong><span>{plan.key}</span><small>v{plan.version} · {plan.versionCount} {text('policy versions', '个策略版本')}</small></button>)}</div>
          ...
          </div> : <div className="entitlement-empty-detail"><ShieldCheck size={20}/><strong>{plans.length ? text('Select a plan', '选择权益方案') : text('No entitlement plans', '暂无权益方案')}</strong><span>{plans.length ? text('Choose a plan to inspect versions and lifecycle controls.', '选择方案以查看版本和生命周期控制。') : text('Create a draft plan to define product access.', '创建草稿方案以定义产品访问权限。')}</span></div>}
        </div>
      </>}
