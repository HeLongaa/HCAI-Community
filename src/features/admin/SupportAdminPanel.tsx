...
export function SupportAdminPanel({ isZh, canRead, canManage }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setReadError(isZh ? '支持工单加载失败。' : 'Could not load support tickets.')
      return false
    } finally { setLoading(false) }
  }, [canRead, isZh, priority, search, slaState, status])
  ...
  const refresh = async () => {
    ...
    try {
      ...
      console.info('[support-admin-detail]', error)
      setReadError(isZh ? '无法读取工单详情。' : 'Could not load ticket detail.')
    }
  }
  ...
  const mutate = async (payload: { status?: SupportTicketStatus; priority?: SupportTicketPriority; assigneeUserId?: string | null }) => {
    ...
    try {
      ...
      setFeedback({ kind: 'success', text: isZh ? '工单已更新。' : 'Ticket updated.' })
    } catch (error) {
      setFeedback({ kind: 'error', text: actionError(error, isZh ? '工单更新失败，可能已被其他管理员修改。' : 'Ticket update failed; it may have changed concurrently.') })
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setFeedback({ kind: 'success', text: isZh ? '回复已发送。' : 'Support reply sent.' })
    } catch (error) {
      setFeedback({ kind: 'error', text: actionError(error, isZh ? '回复发送失败。' : 'Could not send support reply.') })
    } finally { setBusy(false) }
  }
  ...
  const linkCase = async () => {
    ...
    try {
      ...
      setFeedback({ kind: 'success', text: isZh ? '案件已关联。' : 'Case linked.' })
    } catch (error) {
      setFeedback({ kind: 'error', text: actionError(error, isZh ? '案件关联失败，请检查案件 ID。' : 'Could not link the case; check its ID.') })
    } finally { setBusy(false) }
  }
  ...
  return <div className="admin-settings-stack support-admin-workspace" data-testid="support-admin-panel">
    ...
    {selected && <section className="panel support-ticket-detail">
      ...
      <div className="support-detail-controls">
        <label><span>{isZh ? '状态' : 'Status'}</span><select value={selected.status} disabled={!canManage || busy || selected.status === 'closed'} onChange={(event) => void mutate({ status: event.target.value as SupportTicketStatus })}>{statuses.map((value) => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</select></label>
        <label><span>{isZh ? '优先级' : 'Priority'}</span><select value={selected.priority} disabled={!canManage || busy} onChange={(event) => void mutate({ priority: event.target.value as SupportTicketPriority })}><option value="normal">normal</option><option value="urgent">urgent</option></select></label>
        <label><span>{isZh ? '分派用户 ID' : 'Assignee user ID'}</span><div className="inline-actions"><input value={assigneeUserId} onChange={(event) => setAssigneeUserId(event.target.value)} disabled={!canManage || busy} /><button className="icon-button" title={isZh ? '分派' : 'Assign'} type="button" disabled={!canManage || busy} onClick={() => void mutate({ assigneeUserId: assigneeUserId.trim() || null })}><UserRoundCheck size={16} /></button></div></label>
      </div>
      ...
      <div className="support-compose"><textarea rows={3} maxLength={4000} placeholder={isZh ? '回复用户' : 'Reply to requester'} value={message} onChange={(event) => setMessage(event.target.value)} disabled={!canManage || busy || selected.status === 'closed'} /><button className="primary-button" type="button" disabled={!canManage || busy || !message.trim() || selected.status === 'closed'} onClick={() => void sendMessage()}><MessageSquareReply size={16} />{isZh ? '发送' : 'Send'}</button></div>
      <div className="support-case-link"><Link2 size={17} /><select value={caseType} onChange={(event) => setCaseType(event.target.value as typeof caseType)} disabled={!canManage || busy}><option value="moderation_case">moderation case</option><option value="admin_review">admin review</option></select><input placeholder={isZh ? '案件 ID' : 'Case ID'} value={caseId} onChange={(event) => setCaseId(event.target.value)} disabled={!canManage || busy} /><button className="ghost-button" type="button" disabled={!canManage || busy || !caseId.trim()} onClick={() => void linkCase()}>{isZh ? '关联' : 'Link'}</button></div>
    </section>}
  </div>
}
