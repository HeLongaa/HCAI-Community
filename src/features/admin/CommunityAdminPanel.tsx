...
export function CommunityAdminPanel({ hasPermission, isZh }: { hasPermission: (permission: Permission) => boolean; isZh: boolean }) {
  ...
  const load = useCallback(async (cursor: string | null = null, append = false) => {
    ...
    } catch (cause) { if (requestId === loadRequest.current) setReadError(errorText(cause)) } finally { if (requestId === loadRequest.current) setBusy(false) }
  ...
  const previewBulk = async () => {
    setFeedback(null)
    try { const result = await adminService.previewCommunityBulk(targetType, bulkAction, selectedIds); setPreview(result); setConfirmation('') } catch (cause) { setFeedback({ kind: 'error', text: errorText(cause) }) }
  }
  const executeBulk = async () => {
    ...
    try {
      ...
      await load()
      setFeedback({ kind: 'success', text: isZh ? `批量操作完成：成功 ${result.succeededCount}，跳过 ${result.skippedCount}。` : `Bulk completed: ${result.succeededCount} succeeded, ${result.skippedCount} skipped.` })
    } catch (cause) { setFeedback({ kind: 'error', text: errorText(cause) }) } finally { setBusy(false) }
  }
  const exportMetrics = async () => {
    setFeedback(null)
    try {
      const document = await adminService.exportCommunityMetrics()
      downloadJsonArtifact({
        value: document,
        fileName: `community-metrics-${new Date().toISOString().slice(0, 10)}.json`,
        mimeType: 'application/json',
      })
      setFeedback({ kind: 'success', text: isZh ? '社区指标已导出。' : 'Community metrics exported.' })
    } catch (cause) { setFeedback({ kind: 'error', text: errorText(cause) }) }
  }
  const switchTarget = (next: AdminCommunityTargetType) => {
    setTargetType(next); setSearch(''); setDeletionState('active'); setModerationState(''); setSelected(null); setSelectedIds([]); setPreview(null); setConfirmation(''); setFeedback(null); setReadError(null)
  }

  if (!canRead) return <section className="panel task-admin-panel community-admin-panel" data-testid="community-admin-panel"><div className="empty-state"><strong>{isZh ? '无社区管理权限' : 'Community access denied'}</strong></div></section>
  return <section className="panel task-admin-panel community-admin-panel" data-testid="community-admin-panel">
    ...
    <AdminActionFeedback message={readError ? { kind: 'error', text: readError } : null} className="community-admin-read-feedback" />
    <AdminActionFeedback message={feedback} className="community-admin-operation-feedback" />
