...
const downloadJson = (value: unknown) => {
  downloadJsonArtifact({
    value,
    fileName: `risk-cases-${new Date().toISOString().slice(0, 10)}.json`,
    mimeType: 'application/json',
  })
}

export function RiskAdminPanel({ view, t, canRead, canManage, canExport }: Props) {
  ...
  const savePolicy = async () => {
    ...
    try {
      ...
      setPolicy(updated)
      setDraft(updated)
      setFeedback({ kind: 'success', text: textFor(t, 'Risk policy saved.', '风控策略已保存。') })
    } catch (cause) {
      const message = errorMessage(cause, textFor(t, 'Could not save risk policy.', '无法保存风控策略。'))
      await load(false)
      setError(message)
    } finally {
      setBusy(false)
  ...
  const transition = async () => {
    ...
    try {
      ...
      await load(false)
      setFeedback({ kind: 'success', text: textFor(t, 'Risk case transitioned.', '风控案件已完成状态转换。') })
    } catch (cause) {
      const message = errorMessage(cause, textFor(t, 'Could not transition risk case.', '无法转换风控案件状态。'))
      await load(false)
      setError(message)
    } finally {
      setBusy(false)
  ...
  const exportCases = async () => {
    ...
    try {
      ...
      setFeedback({ kind: 'success', text: textFor(t, 'Risk evidence exported.', '风控证据已导出。') })
    } catch (cause) {
      setError(errorMessage(cause, textFor(t, 'Could not export risk evidence.', '无法导出风控证据。')))
    }
  }

  if (!canRead) return <section className="trust-workspace-surface"><div className="empty-state"><ShieldAlert size={20}/><strong>{textFor(t, 'Risk access denied', '无风控读取权限')}</strong></div></section>

  const heading = {
    cases: [textFor(t, 'Risk cases', '风控案件'), textFor(t, 'Account and generation abuse', '账号与生成滥用')],
    policies: [textFor(t, 'Risk policy', '风控策略'), textFor(t, 'Detection thresholds and restrictions', '检测阈值与限制策略')],
    evidence: [textFor(t, 'Risk evidence', '风控证据'), textFor(t, 'Signals, appeals, and status history', '信号、申诉与状态历史')],
  }[view]

  return <section className="trust-workspace-surface risk-admin-panel" data-view={view} data-testid="risk-admin-panel">
    <SectionHeader eyebrow={heading[0]} title={heading[1]} action={<div className="button-row"><button className="icon-button" type="button" title={textFor(t, 'Refresh risk operations', '刷新风控运营')} onClick={() => void load(false)} disabled={busy}><RefreshCw size={16}/></button>{view === 'evidence' && <button className="icon-button" type="button" title={textFor(t, 'Export risk evidence', '导出风控证据')} onClick={() => void exportCases()} disabled={!canExport || busy}><Download size={16}/></button>}</div>}/>
    {error && <div className="inline-alert error">{error}</div>}
    <AdminActionFeedback message={feedback} />
    {view === 'cases' && metrics && <div className="risk-admin-metrics" data-testid="risk-admin-metrics"><div><span>{textFor(t, 'Restricted', '受限')}</span><strong>{metrics.byStatus.restricted ?? 0}</strong></div><div><span>{textFor(t, 'Appealed', '申诉中')}</span><strong>{metrics.byStatus.appealed ?? 0}</strong></div><div><span>{textFor(t, 'Pending appeals', '待审申诉')}</span><strong>{metrics.pendingAppeals}</strong></div><div><span>{textFor(t, 'Critical', '严重')}</span><strong>{metrics.byRiskLevel.critical ?? 0}</strong></div></div>}
    {view === 'policies' && draft && <div className="risk-admin-policy" data-testid="risk-admin-policy">
    ...
    {view !== 'policies' && <>
      ...
      {selected && <div className="admin-detail-panel" data-testid="risk-case-detail"><div className="admin-section-heading"><div><strong>{selected.id}</strong><small>{selected.signals.length} signals · {selected.appeals.length} appeals · v{selected.version}</small></div></div><div className="trust-fact-list">{selected.signals.map((signal) => <div key={signal.id}><ShieldAlert size={15}/><span>{signal.signalType} · {signal.reasonCode}</span><code>{signal.score}</code></div>)}{selected.events.map((event) => <div key={event.id}><ShieldCheck size={15}/><span>{event.fromStatus ?? 'new'} → {event.toStatus} · {event.disposition}</span><small>{event.reasonCode}</small></div>)}{selected.appeals.map((appeal) => <div key={appeal.id}><ShieldCheck size={15}/><span>appeal · {appeal.status}</span><small>{appeal.reasonCode}</small></div>)}</div>{view === 'cases' && transitionTargets[selected.status].length > 0 && <div className="trust-action-grid"><label><span>{textFor(t, 'Next status', '下一状态')}</span><select aria-label={textFor(t, 'Next status', '下一状态')} value={toStatus} onChange={(event) => { const next = event.target.value as RiskCaseStatus; setToStatus(next); if (['recovered', 'closed'].includes(next)) setNextDisposition('cleared') }}>{transitionTargets[selected.status].map((item) => <option key={item}>{item}</option>)}</select></label><label><span>{textFor(t, 'Disposition', '处置')}</span><select aria-label={textFor(t, 'Disposition', '处置')} value={nextDisposition} onChange={(event) => setNextDisposition(event.target.value as RiskDisposition)}>{dispositions.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>{textFor(t, 'Risk level', '风险等级')}</span><select aria-label={textFor(t, 'Risk level', '风险等级')} value={riskLevel} onChange={(event) => setRiskLevel(event.target.value as RiskLevel)}>{levels.map((item) => <option key={item}>{item}</option>)}</select></label>{selected.appeals.some((item) => item.status === 'pending') && <label><span>{textFor(t, 'Appeal decision', '申诉裁决')}</span><select aria-label={textFor(t, 'Appeal decision', '申诉裁决')} value={appealDecision} onChange={(event) => setAppealDecision(event.target.value as 'approved' | 'rejected')}><option value="approved">approved</option><option value="rejected">rejected</option></select></label>}<label className="wide"><span>{textFor(t, 'Reason code', '原因码')}</span><input aria-label={textFor(t, 'Reason code', '原因码')} value={reasonCode} onChange={(event) => setReasonCode(event.target.value)}/></label><button className="primary-button" type="button" onClick={() => void transition()} disabled={!canManage || busy}>{textFor(t, 'Apply transition', '执行状态转换')}</button></div>}</div>}
    </>}
  </section>
}
