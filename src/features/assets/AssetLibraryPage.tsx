...
export function AssetLibraryPage({ t, signedIn, requireAuth, navigateToPage }: {
  ...
  return <main className="asset-library-page" data-testid="asset-library">
    ...
    <section className="asset-library-workbench">
      ...
      {selected && (
        <UseCreativeAsset t={t} assetId={selected.id} fileName={selected.fileName} available={!selected.archivedAt && selected.status === 'uploaded' && selected.scanStatus === 'clean' && Boolean(selected.sourceGeneration)}/>
      )}
    </section>
  </main>
}
