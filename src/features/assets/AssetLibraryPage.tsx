  ...
  const renderAssetDetail = () => {
    ...
    const detail = <aside aria-label={textFor(t, 'Asset details', '素材详情')} className="asset-detail">
      ...
      <div className={`asset-detail-preview ${selected.mediaType}${previewAvailable ? ' has-media' : ''}`}>
        {previewAvailable && selected.mediaType === 'image' ? <img alt="" onError={markPreviewFailed} src={previewUrl}/> : previewAvailable && selected.mediaType === 'video' ? <video aria-label={textFor(t, 'Asset video preview', '资产视频预览')} controls playsInline preload="metadata" onError={markPreviewFailed} src={previewUrl}/> : <Icon data-testid={previewUrl ? `asset-detail-preview-fallback-${selected.id}` : undefined} size={42}/>}<span>{mediaLabel(selected.mediaType)}</span>
      </div>
      <div className="asset-detail-title"><div><small>{purposeLabel(selected.purpose)}</small><h2 title={selected.fileName}>{selected.fileName}</h2><span className={`asset-detail-state ${selectedStateKind}`}><SelectedStateIcon className={selectedStateKind === 'pending' ? 'spin' : ''} size={12}/>{statusLabel(selected)}</span></div></div>
