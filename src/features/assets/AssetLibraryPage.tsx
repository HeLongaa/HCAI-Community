...
import { AlertTriangle, Archive, ArchiveRestore, ArrowDownToLine, Boxes, ChevronRight, ExternalLink, File, FileAudio, FileImage, FileVideo, FolderSearch, LoaderCircle, RefreshCw, Search, Send, ShieldCheck, Trash2, Undo2, Upload } from 'lucide-react'
...
const emptyFilters: Filters = { search: '', mediaType: '', purpose: '', lifecycle: 'active', dateFrom: '', dateTo: '', groupBy: 'mediaType' }
...
export function AssetLibraryPage({ t, signedIn, requireAuth, navigateToPage }: {
  ...
  const renderAssetCard = (asset: ApiAssetLibraryItem) => {
    ...
    return <button className={selected?.id === asset.id ? 'asset-card selected' : 'asset-card'} key={asset.id} onClick={() => { setSelected(asset); setDeleteConfirmId(null) }} type="button"><span className={`asset-thumb ${asset.mediaType}`}><Icon size={24}/></span><span className="asset-card-copy"><strong>{asset.fileName}</strong><small>{asset.mediaType} · {formatBytes(asset.sizeBytes)}</small><span><ShieldCheck size={12}/>{asset.scanStatus}{asset.deletedAt ? ` · ${textFor(t, 'deleted', '回收站')}` : asset.archivedAt ? ` · ${textFor(t, 'archived', '已归档')}` : ''}</span></span><ChevronRight size={15}/></button>
  }
  ...
  return <main className="asset-library-page" data-testid="asset-library">
    <header className="asset-library-header"><div><span><Boxes size={14}/> {textFor(t, 'Creative system of record', '创作资产事实源')}</span><h1>{textFor(t, 'Assets', '资产库')}</h1><p>{textFor(t, 'Search, trace, archive, and reuse governed outputs across studios.', '跨工作台搜索、追溯、归档和复用受治理的创作产物。')}</p></div><div className="asset-library-header-actions"><label className="primary-button asset-upload-button"><Upload size={16}/>{uploading ? textFor(t, 'Preparing…', '准备中…') : textFor(t, 'Upload', '上传')}<input aria-label={textFor(t, 'Upload asset', '上传资产')} disabled={uploading || !online} type="file" onChange={(event) => { void uploadFile(event.target.files?.[0] ?? null); event.target.value = '' }}/></label><button className="icon-button" aria-label={textFor(t, 'Refresh assets', '刷新资产')} onClick={() => void load()} type="button"><RefreshCw className={loading ? 'spin' : ''} size={17}/></button></div></header>
    ...
    <section className="asset-library-filters" aria-label={textFor(t, 'Asset filters', '资产筛选')}>
      ...
      <select aria-label={textFor(t, 'Lifecycle state', '生命周期状态')} value={filters.lifecycle} onChange={(event) => setFilters((value) => ({ ...value, lifecycle: event.target.value as Filters['lifecycle'] }))}><option value="active">{textFor(t, 'Active', '使用中')}</option><option value="archived">{textFor(t, 'Archived', '已归档')}</option><option value="deleted">{textFor(t, 'Trash', '回收站')}</option><option value="all">{textFor(t, 'All', '全部')}</option></select>
