...
type Filters = { search: string; mediaType: '' | AssetMediaType; purpose: '' | MediaAssetPurpose; archived: 'active' | 'archived' | 'all'; dateFrom: string; dateTo: string }
const emptyFilters: Filters = { search: '', mediaType: '', purpose: '', archived: 'active', dateFrom: '', dateTo: '' }
...
export function AssetLibraryPage({ t, signedIn, requireAuth, navigateToPage }: {
  ...
  return <main className="asset-library-page" data-testid="asset-library">
    ...
    <section className="asset-library-workbench">
      <div className="asset-grid" aria-busy={loading}>{loading && items.length === 0 ? <div className="asset-empty"><LoaderCircle className="spin"/><span>{textFor(t, 'Loading assets…', '正在加载资产…')}</span></div> : items.length === 0 ? <div className="asset-empty"><FolderSearch/><strong>{textFor(t, 'No assets found', '没有找到资产')}</strong><span>{textFor(t, 'Adjust filters or create something in a studio.', '调整筛选条件或前往工作台创作。')}</span></div> : items.map((asset) => { const Icon = icons[asset.mediaType]; return <button className={selected?.id === asset.id ? 'asset-card selected' : 'asset-card'} key={asset.id} onClick={() => setSelected(asset)} type="button"><span className={`asset-thumb ${asset.mediaType}`}><Icon size={24}/></span><span className="asset-card-copy"><strong>{asset.fileName}</strong><small>{asset.mediaType} · {formatBytes(asset.sizeBytes)}</small><span><ShieldCheck size={12}/>{asset.scanStatus}{asset.archivedAt && ` · ${textFor(t, 'archived', '已归档')}`}</span></span><ChevronRight size={15}/></button>})}{nextCursor && <button className="asset-load-more" disabled={loading} onClick={() => void load(nextCursor)} type="button">{textFor(t, 'Load more', '加载更多')}</button>}</div>
