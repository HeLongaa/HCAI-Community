  ...
  const setUserRole = (nextRole: Role | ((prevState: Role) => Role)) => {
    setAccount((current) => {
      ...
      const permissions = role === 'admin'
        ? ['task:create', 'task:propose', 'task:claim', 'task:submit', 'task:review', 'task:moderate', 'post:create', 'post:moderate', 'comment:create', 'points:read', 'points:adjust', 'admin:access', 'admin:audit:read', 'admin:queue:read', 'admin:queue:review', 'admin:permissions:manage', 'admin:auth:read', 'admin:auth:manage', 'security:alerts:manage'] satisfies Permission[]
        : current.permissions
