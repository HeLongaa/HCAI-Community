...
const adminTabs = new Set<NonNullable<AdminDeepLink['tab']>>(['Task review', 'Access', 'Security', 'Finance', 'Generations', 'Submissions', 'Community', 'Audit log', 'Users', 'Tags', 'AI config'])
...
const parseAdminTarget = (value: unknown): AdminDeepLink | undefined => {
  ...
  for (const key of ['queue', 'reviewId', 'auditEventId', 'ledgerUserHandle', 'policyHistoryEventId', 'securityAlertId', 'mediaAssetId'] as const) {
...
export const notificationTargetHash = (target: NotificationTargetV1) => {
  ...
  if (target.surface === 'admin' && target.admin) {
    for (const key of ['tab', 'queue', 'reviewId', 'auditEventId', 'ledgerUserHandle', 'policyHistoryEventId', 'securityAlertId', 'mediaAssetId', 'mediaStatus'] as const) {
