...
Current decision: **the lifecycle and guarded Router client are registered but disabled by default**. Fixture injection
remains supported. Real status polling and private Router content reads require the complete staging runtime envelope;
Router cancellation is unsupported and fails explicitly. There is no production Provider path.
...
- Polling is enabled only by independent staging `CREATIVE_ROUTER_VIDEO_LIFECYCLE_*` switches and either the fixture or
  real staging confirmation. Real staging also requires the HTTP/network gates and credentials.
- Missing clients fail closed. The dedicated worker constructs the Router client only when the real lifecycle worker
  gate is enabled.
...
also proves Router cancellation returns an unsupported error without changing job or billing state. The gate retains
fixture coverage and verifies that the real client remains default-off. Real staging requires credentials and
