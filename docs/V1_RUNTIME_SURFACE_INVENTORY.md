...
| `server-video-capability-contract` | The Video contract, governed input/byte/lineage resolver, fixture-only Veo request mapper, strict async projection/replay builder, and generated-second budget reservation are executable while Veo and Runway remain disabled catalog shells | Retain the fixture-only boundary and keep adapter registration, HTTP clients, Provider state, lifecycle registration, output fetch, and production enablement disabled until V1-27 through V1-29 close | V1-25 through V1-29, V1-39 |
