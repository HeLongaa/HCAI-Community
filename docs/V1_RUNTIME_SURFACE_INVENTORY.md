...
| `server-provider-control-plane` | Versioned kill switches, expiring hash-only cap evidence, explicit circuits, immutable AI quality/safety regression evidence, one-claim probes, two-person recovery, and safe metrics gate fixture dispatch | Retain the fail-closed app control plane; keep real Provider clients, cap readers, probes, pricing, and dispatch unregistered until approval | V1-11 |
