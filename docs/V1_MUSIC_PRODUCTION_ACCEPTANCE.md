...
- `server/src/creative/musicProductionAcceptance.test.js` proves the fixed `mp3_44100_256` quality profile, one-output
  and three-minute limits, rights/license requirements, request-based spend limits, daily blocking before dispatch,
  and Router MiniMax staging-client rollback without network dispatch or automatic Lyria/Mock fallback.
- The fixture application acceptance is bounded to five seconds. Provider tests prove strict MP3 validation, bounded
  ingestion, safe failure projection, license evidence, training opt-out, and request-based accounting.
...
Production remains fail-closed. A go decision requires AI-MUSIC-01 credentialed Router MiniMax acceptance, a paid or
