...
2. 目标环境发布/回滚：RELEASE-01 基础设施演练和 RELEASE-02 应用候选/回滚执行器均已在本地通过；后者本地只使用内存 HTTP 夹具。尚无专用 staging 的真实不可变制品部署、流量切换、回滚后冒烟和受保护环境回执，因此不得标记完成。
