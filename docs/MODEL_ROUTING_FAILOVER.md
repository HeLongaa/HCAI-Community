...
`MODEL-02` adds a credential-free routing control plane above the normalized model registry. It does not configure Provider credentials, register a network client, or authorize external traffic.

## Selection

Active policies match modality, operation, environment, optional region, optional account role, and a deterministic percentage bucket. The bucket hashes the policy key, rollout seed, and request subject key. The subject key is used only in memory and is never returned, audited, exported, or persisted.
...
Current deployments remain `trafficEligible=false`, so route preview returns an explicit unavailable result. Real Provider traffic requires the later `MODEL-03`, `LEGAL-BASE-01`, `AI-EVAL-01`, and explicit `PROVIDER-APPROVAL` gates.
