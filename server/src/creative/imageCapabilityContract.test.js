...
test('image provider projection distinguishes supported and unavailable modes', () => {
  ...
  assert.deepEqual(mock.modes, ['text_to_image', 'image_to_image', 'image_edit', 'image_variation'])
  ...
  assert.equal(mock.modeContracts.find((mode) => mode.id === 'image_edit').available, true)
  ...
  assert.deepEqual(openai.modes, ['text_to_image', 'image_to_image', 'image_edit', 'image_variation'])
  assert.deepEqual(openai.supportedParameters, ['aspectRatio', 'stylePreset', 'quality', 'outputCount', 'outputFormat', 'strength'])
...
test('image generation validation rejects unknown and invalid combinations', () => {
  assert.equal(assertImageGenerationRequest({
    ...request,
    mode: 'image_edit',
    inputAssetIds: ['source', 'mask'],
    parameters: { stylePreset: 'none', seed: 42, strength: 0.5, quality: 'medium' },
  }).mode, 'image_edit')
  assert.throws(
