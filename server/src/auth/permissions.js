...
export const permissionRegistry = Object.freeze([
  ...
  definePermission('admin:inspiration:read', 'inspiration-library', 'inspiration_entry', 'read', 'high', moderators, { description: 'Read inspiration catalog, categories, versions, and review queue' }),
  definePermission('admin:inspiration:manage', 'inspiration-library', 'inspiration_entry', 'manage', 'critical', admins, { resourceAuthorization: true, description: 'Manage inspiration categories, official content, review decisions, publication, and archive state' }),
  ...
  definePermission('admin:risk:read', 'trust-safety-risk', 'risk_case', 'read', 'high', moderators, { description: 'Read normalized account and generation risk cases, signals, appeals, and metrics' }),
  definePermission('admin:risk:manage', 'trust-safety-risk', 'risk_case', 'transition', 'critical', admins, { resourceAuthorization: true, description: 'Apply audited account and generation risk dispositions and appeal decisions' }),
  definePermission('admin:risk:export', 'trust-safety-risk', 'risk_case', 'export', 'critical', admins, { description: 'Export bounded normalized risk evidence without credentials, prompts, or Provider payloads' }),
  ...
  definePermission('admin:support:read', 'support-service', 'support_ticket', 'read', 'high', moderators, { description: 'Read bounded support ticket, SLA, assignment, and message projections' }),
  definePermission('admin:support:manage', 'support-service', 'support_ticket', 'manage', 'critical', admins, { protected: true, protectionRank: 34, resourceAuthorization: true, description: 'Assign, prioritize, respond to, link, resolve, and close support tickets' }),
  definePermission('admin:search:read', 'search-discovery', 'search_index', 'read', 'high', moderators, { description: 'Read search index synchronization status and bounded diagnostics' }),
  definePermission('admin:search:manage', 'search-discovery', 'search_index', 'manage', 'critical', admins, { protected: true, protectionRank: 35, resourceAuthorization: true, description: 'Process search synchronization failures and request bounded index rebuilds' }),
  ...
  definePermission('admin:data-rights:read', 'compliance-data-rights', 'data_rights_request', 'read', 'high', moderators, { description: 'Read bounded data export and deletion request evidence and metrics' }),
  definePermission('admin:data-rights:manage', 'compliance-data-rights', 'data_rights_request', 'manage', 'critical', admins, { protected: true, protectionRank: 36, resourceAuthorization: true, description: 'Process data exports, primary deletion, and immutable backup expiry evidence' }),
  definePermission('admin:data-rights:legal-hold', 'compliance-data-rights', 'data_rights_legal_hold', 'manage', 'critical', admins, { protected: true, protectionRank: 37, resourceAuthorization: true, description: 'Create and release scoped expiring legal holds with immutable evidence' }),
