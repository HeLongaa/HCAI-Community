...
test('GET /metrics returns Prometheus metrics for authorized scrapes', async () => {
  ...
  await withProcessEnv({ METRICS_EXPORTER_ENABLED: 'true', METRICS_EXPORTER_TOKEN: 'metrics-secret' }, async () => {
    ...
    try {
      ...
      assert.match(body, /newchat_creative_provider_budget_alerts_total/)
      assert.match(body, /newchat_creative_provider_budget_dispatch_blocked_total/)
      assert.match(body, /newchat_creative_provider_cost_anomalies_total/)
