...
const staticCandidateBlock = (target, policy, context) => {
  ...
  const globalDeployment = deployment.region === 'global'
  if (policy.region && !globalDeployment && deployment.region !== policy.region) return 'deployment_region_mismatch'
  if (context.region && !globalDeployment && deployment.region !== context.region) return 'request_region_mismatch'
