...
const deployment = {
  ...
  runtimeConfig: { providerAccountRef: 'museflow-image-staging', displayName: 'HCAI Router Image', costProviderId: 'hc-router', dailyBudgetUsd: 10, budgetThresholdPercent: 80 },
...
test('active staging route resolves deployment Provider, model, endpoint, and SecretRef without exposing the credential', async () => {
  ...
  assert.equal(resolved.runtimeSource.CREATIVE_OPENAI_IMAGE_DISPLAY_NAME, 'HCAI Router Image')
  assert.equal(resolved.runtimeSource.CREATIVE_OPENAI_IMAGE_COST_PROVIDER_ID, 'hc-router')
