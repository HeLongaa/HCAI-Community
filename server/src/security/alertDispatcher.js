...
const alertWebhookUrl = () => trimValue(process.env.SECURITY_ALERT_WEBHOOK_URL)
const alertWebhookSecret = () => trimValue(process.env.SECURITY_ALERT_WEBHOOK_SECRET)
const alertWebhookTimeoutMs = () => positiveInteger(process.env.SECURITY_ALERT_WEBHOOK_TIMEOUT_SECONDS, 5) * 1000
const slackWebhookUrl = () => trimValue(process.env.SECURITY_ALERT_SLACK_WEBHOOK_URL)
const slackWebhookTimeoutMs = () => positiveInteger(process.env.SECURITY_ALERT_SLACK_TIMEOUT_SECONDS, 5) * 1000
const emailWebhookUrl = () => trimValue(process.env.SECURITY_ALERT_EMAIL_WEBHOOK_URL)
const emailWebhookSecret = () => trimValue(process.env.SECURITY_ALERT_EMAIL_WEBHOOK_SECRET)
const emailRecipients = () => trimValue(process.env.SECURITY_ALERT_EMAIL_TO)
  .split(',')
  .map((item) => item.trim())
  .filter(Boolean)
const emailFrom = () => trimValue(process.env.SECURITY_ALERT_EMAIL_FROM)
const emailWebhookTimeoutMs = () => positiveInteger(process.env.SECURITY_ALERT_EMAIL_TIMEOUT_SECONDS, 5) * 1000
...
const webhookChannel = {
  ...
  dispatch: async (alert) => {
    ...
    const headers = {
      'x-security-alert-id': alert.id,
      'x-security-alert-type': alert.type,
    }
    const secret = alertWebhookSecret()
    if (secret) {
      headers['x-security-alert-signature'] = signAlertPayload(secret, body)
    }
    return requestJson({
      channel: 'webhook',
      url,
  ...
  dispatch: async (alert) => {
    ...
    if (recipients.length === 0) {
      return {
        channel: 'email',
        status: 'failed',
        error: 'SECURITY_ALERT_EMAIL_TO is required',
      }
    }
...
export const dispatchSecurityAlert = async (alert) => {
