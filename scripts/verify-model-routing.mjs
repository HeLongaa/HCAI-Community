...
check(contract.rawSubjectIdentifiersPersisted === false, 'routing persists no raw subject identifiers')
check(contract.subjectHashPersisted === true, 'routing records a stable safe subject hash')
...
check(docs.includes('approved and applied `MODEL-05` production promotion'), 'runbook preserves the production promotion gate')
