...
const retentionMigration = read('server/prisma/migrations/0115_support_ticket_retention/migration.sql')
...
const retentionRuntime = read('server/src/support/supportRetention.js')
const retentionRepo = read('server/src/support/prismaSupportRetentionRepository.js')
...
add('support retention contract is two-stage and fail-closed', contract.retention?.policyId === 'support_close_plus_730d' && contract.retention.messageBodyDays === 365 && contract.retention.minimalEvidenceDays === 730 && contract.retention.workerDefaultEnabled === false)
add('support retention preserves minimal case evidence and removes actor links', retentionRepo.includes('supportTicketCaseLink.updateMany') && retentionRepo.includes('createdById: null') && retentionRepo.includes('requesterId: null') && !retentionRepo.includes('supportTicketCaseLink.deleteMany'))
add('support retention blocks legal holds and active data-rights requests', retentionRuntime.includes("['support', 'audit']") && retentionRepo.includes('dataRightsLegalHold.findFirst') && retentionRepo.includes('dataRightsRequest.findFirst'))
add('support mutations and retention share a transaction lock', prismaRepo.includes('support-ticket:') && retentionRepo.includes('support-ticket:'))
add('support retention migration backfills refs and permits identity unlinking', retentionMigration.includes('requester_subject_ref') && retentionMigration.includes('created_by_subject_ref') && retentionMigration.includes('ON DELETE SET NULL'))
add('support retention uses a default-disabled leased worker', workerJobs.includes("id: 'support-retention-sweep'") && workerJobs.includes("lease: lease('support-retention-sweep')"))
