...
add(
  'event facts and publication state are separate',
  /publication\s+DomainEventPublication\?/.test(schema) && /event\s+DomainEventOutbox\s+@relation/.test(schema),
  'immutable fact plus delivery state',
)
