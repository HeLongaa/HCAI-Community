...
if (compose) {
  ...
  add('Provider egress network permits outbound traffic', compose.networks?.[contract.providerEgressNetwork]?.internal !== true, contract.providerEgressNetwork)
  ...
  const workerNetworks = Object.keys(services.worker?.networks ?? {}).sort()
  add(
    'Worker has backend and dedicated Provider egress without edge access',
    workerNetworks.join(',') === [contract.internalNetwork, contract.providerEgressNetwork].sort().join(','),
    workerNetworks.join(', '),
  )
