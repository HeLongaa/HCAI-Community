...
check(routes.includes('providerTrafficEnabled: false'), 'Admin audit and summary expose disabled traffic state')
...
check(panel.includes('Real Provider traffic disabled'), 'Admin workbench shows the traffic gate')
check(!routes.includes('secretRef') && !routes.includes('credential'), 'registry routes do not accept secrets')
check(docs.includes('Ordinary continuation language is not approval'), 'runbook preserves explicit human approval')
