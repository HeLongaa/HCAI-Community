...
test('fixture Music readiness passes without exposing its credential', () => {
  const result = spawnSync(process.execPath, ['scripts/check-router-music-readiness.mjs', '--profile=fixture', '--mode=preflight'], { cwd: process.cwd(), encoding: 'utf8', env: {} })
...
test('environment Music readiness fails closed without staging evidence', () => {
  const result = spawnSync(process.execPath, ['scripts/check-router-music-readiness.mjs', '--profile=env', '--mode=preflight'], { cwd: process.cwd(), encoding: 'utf8', env: {} })
