...
const runtimeEnvironment = read('server/src/common/runtimeEnvironment.js')
const runtimeConfig = read('server/src/config/env.js')
...
add('Google and GitHub implementation is explicitly approved', contract.realProviderCallsApproved === true && contract.providers.includes('github'), 'Google/GitHub OAuth')
...
add('production dev fallback is disabled', oauth.includes('!isProductionEnvironment(source)') && oauth.includes("mode = configured ? 'external' : isOAuthDevModeEnabled(source) ? 'dev' : 'unavailable'"), 'explicit unavailable mode')
add('production signals are unified and inconsistent deployment fails closed', runtimeEnvironment.includes('source.NODE_ENV') && runtimeEnvironment.includes('source.DEPLOYMENT_ENV') && runtimeConfig.includes('DEPLOYMENT_ENV=production requires NODE_ENV=production'), 'NODE_ENV + DEPLOYMENT_ENV')
add('canonical callback and trusted browser return origins are required', oauth.includes('OAUTH_CALLBACK_ORIGIN') && oauth.includes('OAUTH_BROWSER_RETURN_ORIGIN') && oauth.includes('getTrustedOrigins(source).includes(browserReturnOrigin)'), 'callback/return origins')
add('Google, GitHub, and Discord use S256 PKCE', oauth.includes("const pkceProviders = new Set(['google', 'github', 'discord'])") && oauth.includes("method: 'S256'"), 'google/github/discord S256')
...
add('all Provider emails require verification evidence', oauth.includes('profile.email_verified !== true') && oauth.includes('profile.verified !== true') && oauth.includes('entry?.verified === true') && oauth.includes('emailVerified'), 'verified email')
...
add('split-origin callback returns directly to the product route', routes.includes('buildOAuthBrowserReturnUrl') && routes.includes('window.location.replace(payload.returnUrl)'), 'frontend hash return')
