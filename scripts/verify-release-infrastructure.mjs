...
add('remote runner verifies exact source and uses isolated services', remoteRunner.includes('checkout --quiet --detach "$source_sha"') && remoteRunner.includes('newchat-release-rehearsal') && remoteRunner.includes('release-infrastructure.env') && remoteRunner.includes('cd "$source_dir"'), 'dedicated source, working directory, Compose project, and runtime')
